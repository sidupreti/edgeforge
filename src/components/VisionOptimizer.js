...
const kb = (v) => {
  ...
  if (v >= 1024) return `${(v / 1024).toFixed(1).replace(/\.0$/, "")}M`;   // 3686 KB -> 3.6M, not 4M
...
export default function VisionOptimizer() {
  ...
  async function run(useSample) {
    ...
    try {
      ...
      setError(String(e.message || e).includes("Failed to fetch")
        ? `Can't reach the optimizer service at ${FORGEOPT_URL}. Start it: uvicorn web.api.server:app --port 8100 (in the forgeopt repo).`
        : String(e.message || e));
    }
    setBusy(false);
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#ffffff", color: INK }}>
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "38px 24px 80px" }}>
        ...
        <p style={{ color: SUB, fontSize: 15, lineHeight: 1.55, margin: "0 0 30px", maxWidth: 520 }}>
          Bring a trained CNN in ONNX — from YOLO, PyTorch, or TensorFlow. We make it smaller and
          faster, measure the accuracy on your data, and tell you honestly whether it still works.
        </p>
        ...
        <p style={{ color: MUTE, fontSize: 12, textAlign: "center", marginTop: 26 }}>
          Runs the ForgeOpt engine. Numbers are measured on the host unless marked “estimate.”
        </p>
      </div>
    </div>
  );
...
function Report({ report }) {
  ...
  return (
    <div style={{ border: `1px solid ${LINE}`, borderRadius: 16, padding: 24, marginTop: 22, background: "#fff" }}>
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <table className="vo-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            ...
            {report.cost.map((r, i) => (
              <tr key={i} style={{ color: r.fits ? INK : MUTE }}>
                ...
                <td style={{ padding: "9px 10px", borderBottom: `1px solid ${LINE}88`, fontFamily: MONO,
                  color: r.flash_kb > r.flash_budget_kb ? RED : undefined,
                  fontWeight: r.flash_kb > r.flash_budget_kb ? 700 : undefined }}>{kb(r.flash_kb)}/{kb(r.flash_budget_kb)}</td>
                <td style={{ padding: "9px 10px", borderBottom: `1px solid ${LINE}88`, fontFamily: MONO,
                  color: r.ram_kb > r.ram_budget_kb ? RED : undefined,
                  fontWeight: r.ram_kb > r.ram_budget_kb ? 700 : undefined }}>{kb(r.ram_kb)}/{kb(r.ram_budget_kb)}</td>
      ...
      <a className="vo-dl" href={`${FORGEOPT_URL}${report.download_url}`} download
        style={{ display: "inline-block", background: INK, color: "#fff", textDecoration: "none",
          padding: "11px 20px", borderRadius: 10, fontWeight: 600, fontSize: 14, transition: "all .15s" }}>
